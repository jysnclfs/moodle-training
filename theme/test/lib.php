...
function theme_test_get_pre_scss($theme) {
...
// Note the function name starts with the component name because this is a global function and we don't want namespace clashes.
function theme_test_get_extra_scss($theme) {
    // Load the settings from the parent.                                                                                           
    $theme = theme_config::load('boost');                                                                                           
...
function theme_test_get_main_scss_content($theme) {                                                                                
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_test', 'preset', 0, '/', $filename))) {              
        // This preset file was fetched from the file area for theme_test and not theme_boost (see the line above).                
...
function theme_test_update_settings_images($settingname) {                                                                         
    global $CFG;                                                                                                                    
 
    // The setting name that was updated comes as a string like 's_theme_test_loginbackgroundimage'.                               
    ...
    if ($file = $fs->get_file_by_hash(sha1($fullpath))) {                                                                           
        ...
        $pathname = $CFG->dataroot . '/pix_plugins/theme/test/' . $settingname . '.' . $extension;                                 
 
        // This pattern matches any previous files with maybe different file extensions.                                            
        $pathpattern = $CFG->dataroot . '/pix_plugins/theme/test/' . $settingname . '.*';                                          
 
        // Make sure this dir exists.                                                                                               
        @mkdir($CFG->dataroot . '/pix_plugins/theme/test/', $CFG->directorypermissions, true);                                      
